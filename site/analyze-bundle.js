...
function analyzeBundle() {
  ...
  chunks.slice(0, 10).forEach((chunk, index) => {
    console.log(`${(index + 1).toString().padStart(2)}: ${chunk.file.padEnd(35)} ${chunk.formattedSize.padStart(10)}`);
  });
  ...
  const threeChunks = chunks.filter(chunk => 
  ...
  console.log(`Three.js % of Total:      ${((threeSize / totalSize) * 100).toFixed(1)}%`);
