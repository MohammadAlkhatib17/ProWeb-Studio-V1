...
function getBundleStats() {
  ...
  if (!fs.existsSync(statsPath)) {
    console.error(
      "Bundle analyzer stats not found. Make sure to run build with ANALYZE=true",
    );
    process.exit(1);
  }
...
function categorizeChunk(filename) {
  ...
  if (filename.includes("vendors") || filename.includes("framework"))
    return "vendors";
  if (filename.includes("three") || filename.startsWith("three."))
    return "three-core";
...
function generateAnalysisData() {
  ...
  const totalBundleSize = chunkFiles.reduce(
  ...
  const analysisData = {
    analysisMetadata: {
      ...
      nextJsVersion:
        packageJson.dependencies.next?.replace("^", "") || "unknown",
      bundleAnalyzerVersion:
        packageJson.devDependencies["@next/bundle-analyzer"]?.replace(
          "^",
          "",
        ) || "unknown",
