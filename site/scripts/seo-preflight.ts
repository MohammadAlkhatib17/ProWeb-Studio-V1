...
class SEOPreflightChecker {
  ...
  private async startServer(): Promise<ServerProcess> {
    return new Promise((resolve, reject) => {
      ...
      const nextProcess = spawn(
        "npx",
        ["next", "start", "-p", SERVER_PORT.toString()],
        {
      ...
      const timeout = setTimeout(() => {
        if (!serverReady) {
          nextProcess.kill();
          reject(
            new Error(`Server failed to start within ${STARTUP_TIMEOUT}ms`),
          );
        }
      }, STARTUP_TIMEOUT);
      ...
      nextProcess.stdout.on("data", (data) => {
        ...
        if (
          ...
          output.includes(`http://localhost:${SERVER_PORT}`)
        ) {
          serverReady = true;
          clearTimeout(timeout);
  ...
  private async fetchPage(
    ...
    try {
      ...
      throw new Error(
        `Failed to fetch ${route}: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  private parseJsonLd(html: string): JsonLdHelper {
    ...
    jsonLdScripts.each((_, script) => {
      try {
        ...
        console.warn(
          `⚠️  Failed to parse JSON-LD: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    });
  ...
  private checkStructuredData(route: string, html: string): void {
    ...
    if (webPageNodes.length !== 1) {
      ...
      this.results.push({
        ...
        message: hasValidId
          ? undefined
          : `WebPage @id should end with #webpage, found: ${webPageData["@id"]}`,
      });
      ...
      if (webPageData.primaryImageOfPage) {
        ...
        if (imageNode) {
          ...
          this.results.push({
            ...
            message: hasValidImageId
              ? undefined
              : `Primary image @id should end with #primaryimage, found: ${imageNode["@id"]}`,
          });
        }
      }
    }
      ...
      if (serviceNodes.length !== 3) {
        ...
        const validServices = serviceNodes.filter((service) => {
          ...
          return (
            ...
            (offer.url?.includes("#website") ||
              offer.url?.includes("#webshop") ||
              offer.url?.includes("#seo"))
          );
        });
        ...
        this.results.push({
          ...
          message:
            validServices.length === 3
              ? undefined
              : `Expected 3 valid Service offers, found ${validServices.length}`,
        });
      }
      ...
      if (catalogNodes.length === 0) {
        ...
        this.results.push({
          ...
          message:
            itemCount === 3
              ? undefined
              : `Expected 3 items in OfferCatalog, found ${itemCount}`,
        });
      }
    }
    ...
    if (orgNodes.length > 0 && businessNodes.length > 0) {
      ...
      this.results.push({
        ...
        message: sameId
          ? undefined
          : `Organization @id: ${orgData["@id"]}, LocalBusiness @id: ${businessData["@id"]}`,
      });
  ...
  private validateServiceArea(
    ...
    if (Array.isArray(serviceArea)) {
      const validRegions = serviceArea.filter(
        (region) =>
      ...
      this.results.push({
        ...
        message:
          validRegions.length === 12
            ? undefined
            : `Expected 12 DefinedRegion with addressCountry: "NL", found ${validRegions.length}`,
      });
    } else {
      this.results.push({
        route,
  ...
  private validateContactPoint(
    ...
    const contacts = Array.isArray(contactPoint)
      ? contactPoint
      : [contactPoint];
    const salesContact = contacts.find(
      (contact) => contact.contactType === "sales",
    );
    ...
    const hasValidLanguages =
      salesContact.availableLanguage &&
    ...
    this.results.push({
      ...
      message: hasValidLanguages
        ? undefined
        : 'Sales contactPoint should have availableLanguage: ["nl", "en"]',
    });
    ...
    this.results.push({
      ...
      message: hasAreaServed
        ? undefined
        : 'Sales contactPoint should have areaServed: "NL"',
    });

    // Check URL to /contact
    const hasContactUrl =
      salesContact.url && salesContact.url.includes("/contact");
    this.results.push({
      route,
      check: `${entityType} contactPoint URL`,
      status: hasContactUrl ? "pass" : "fail",
      message: hasContactUrl
        ? undefined
        : "Sales contactPoint should have URL pointing to /contact",
    });
  }
   ...
   */
  private validatePotentialAction(
    route: string,
    data: JsonLdNode,
    entityType: string,
  ): void {
    ...
    const scheduleAction = actions.find(
      (action) => action["@type"] === "ScheduleAction",
    );
    ...
    const hasValidUrlTemplate =
      target.urlTemplate && target.urlTemplate.includes("/contact");
    this.results.push({
      route,
      check: `${entityType} ScheduleAction urlTemplate`,
      status: hasValidUrlTemplate ? "pass" : "fail",
      message: hasValidUrlTemplate
        ? undefined
        : "EntryPoint urlTemplate should end with /contact",
    });
  }
  ...
  private async checkSitemap(): Promise<void> {
    try {
      const { html: sitemapContent, status } =
        await this.fetchPage("/sitemap.xml");
      ...
      this.results.push({
        ...
        message: hasFragments
          ? "Sitemap contains URLs with # fragments"
          : undefined,
      });
      ...
      this.results.push({
        ...
        message:
          httpUrls.length === 0
            ? undefined
            : `Found ${httpUrls.length} HTTP URLs`,
      });
      ...
      this.results.push({
        ...
        message:
          relativeUrls.length === 0
            ? undefined
            : `Found ${relativeUrls.length} relative URLs`,
      });
    } catch (error) {
      this.results.push({
        route: "/sitemap.xml",
  ...
  private async checkRobotsTxt(): Promise<void> {
    try {
      const { html: robotsContent, status } =
        await this.fetchPage("/robots.txt");
      ...
      if (status === 200) {
        const hasUserAgent = robotsContent
          .toLowerCase()
          .includes("user-agent:");
        this.results.push({
          route: "/robots.txt",
  ...
  private async checkInternalAnchors(): Promise<void> {
    ...
    try {
      ...
      this.results.push({
        ...
        message:
          foundIds.length === 3
            ? undefined
            : `Missing IDs: [${requiredIds.filter((id) => !foundIds.includes(id)).join(", ")}]`,
      });
    } catch (error) {
      this.results.push({
        route: "/diensten",
    ...
    try {
      ...
      const anchorLinks = $('a[href*="/diensten#"]')
        .toArray()
        .map((el) => $(el).attr("href"))
        .filter(
          (href) =>
  ...
  private checkRobotsMeta(route: string, html: string): void {
    ...
    if (isPreviewOrStaging) {
      ...
      const hasNoIndex =
        robotsMeta && robotsMeta.toLowerCase().includes("noindex");
      this.results.push({
        route,
        check: "Robots meta (no accidental noindex)",
        status: hasNoIndex ? "fail" : "pass",
        message: hasNoIndex
          ? `Found noindex on canonical page: ${robotsMeta}`
          : undefined,
      });
    } else {
  ...
  private printResults(): void {
    ...
    if (failedChecks.length > 0) {
      ...
      console.log("\n🔧 Debugging Tips:");
      console.log(
        "  • Schema type counts are logged for each route during execution",
      );
      console.log(
        "  • Failed singleton checks show @id values to identify duplication sources",
      );
      console.log(
        "  • Check server logs above for additional JSON-LD parsing warnings",
      );
    }
  }
      ...
      if (this.serverProcess) {
        ...
        try {
          ...
          try {
            ...
            console.error(
              "⚠️ pkill fallback failed:",
              pkillError instanceof Error
                ? pkillError.message
                : "Unknown error",
            );
          }
