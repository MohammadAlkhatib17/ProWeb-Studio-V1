...
function validateRouteConfig(routeFile) {
  ...
  const regionMatch = content.match(/export\s+const\s+preferredRegion\s*=\s*['"]([^'"]+)['"]/);
...
function main() {
  ...
  results.forEach(result => {
    ...
    const overallStatus = (result.hasCorrectRuntime && result.hasCorrectRegion) ? '✅ Valid' : '❌ Invalid';
    
    console.log(`| /api${result.route} | ${result.runtime || 'missing'} ${runtimeIcon} | ${result.region || 'missing'} ${regionIcon} | ${overallStatus} |`);
