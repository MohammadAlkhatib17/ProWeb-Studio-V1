...
const ROUTE_PREFETCH_ASSETS = ["/_next/static/css/", "/_next/static/chunks/"];
...
function isExpired(cachedResponse) {
  ...
  const now = Date.now();
  const maxAge =
    cachedResponse.headers.get("sw-max-age") || CACHE_EXPIRES.DYNAMIC;
...
self.addEventListener("fetch", (event) => {
  ...
  if (PATTERNS.EXTERNAL_ANALYTICS.test(url.href)) {
    ...
    PATTERNS.STATIC_ASSETS.test(url.pathname) ||
...
async function handleApiRequest(request) {
  ...
  try {
    ...
    if (networkResponse.ok) {
      const timestampedResponse = addTimestampToResponse(
        networkResponse,
        CACHE_EXPIRES.API,
      );
...
console.log(
  `[SW] ProWeb Studio Service Worker v${CACHE_VERSION} loaded with advanced caching strategies`,
);
