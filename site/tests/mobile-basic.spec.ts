...
test.describe("Mobile Core Tests", () => {
  ...
  test("iPhone 12 - Mobile menu scroll lock basic test", async ({ page }) => {
    ...
    try {
      ...
      console.log(
        "Mobile menu visibility test skipped due to timing/hydration issue",
      );
  ...
  test("iPhone 12 - Canvas basic visibility and bounds", async ({ page }) => {
    ...
    expect(canvasBox!.x + canvasBox!.width).toBeLessThanOrEqual(
      IPHONE_12.width + viewportBuffer,
    );
    expect(canvasBox!.y + canvasBox!.height).toBeLessThanOrEqual(
      IPHONE_12.height + viewportBuffer,
    );
    ...
    expect(
      ["absolute", "relative", "fixed", "static"].includes(
        canvasStyles.position,
      ),
    ).toBeTruthy();
  });
  ...
  test("Pixel 5 - Canvas presence on services page", async ({ page }) => {
    ...
    if (canvasCount > 0) {
      ...
      expect(canvasBox!.x + canvasBox!.width).toBeLessThanOrEqual(
        PIXEL_5.width + generousBuffer,
      );
      expect(canvasBox!.y + canvasBox!.height).toBeLessThanOrEqual(
        PIXEL_5.height + generousBuffer,
      );
    } else {
