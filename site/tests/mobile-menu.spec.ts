...
test.describe("Mobile Menu Scroll Lock Tests", () => {
  ...
  test("iPhone 12 - Home page mobile menu scroll lock at top", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    const scrollLockState = await page.evaluate(() => {
      ...
      const hasOverflowHidden =
        bodyStyles.overflow === "hidden" || htmlStyles.overflow === "hidden";
    ...
    expect(
      scrollLockAfter.bodyInlineOverflow === "" ||
        scrollLockAfter.bodyInlineOverflow === "auto" ||
        scrollLockAfter.bodyInlineOverflow === "visible",
    ).toBeTruthy();
  });
  ...
  test("Pixel 5 - Services page mobile menu scroll lock", async ({ page }) => {
    ...
    const scrollContainer = await page.evaluate(() => {
      ...
      const htmlLocked =
        htmlStyles.overflow === "hidden" || html.style.overflow === "hidden";
  ...
  test("iPhone 12 - Services page mobile menu at mid-scroll", async ({
    page,
  }) => {
    await page.goto("/diensten");
    await page.waitForLoadState("networkidle");
