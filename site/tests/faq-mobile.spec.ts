...
test.describe("FAQ Mobile Accordion Functionality", () => {
  ...
  test("should toggle FAQ items and maintain accessibility", async ({
    page,
  }) => {
  ...
  test("should maintain rectangular card layout on mobile", async ({
    page,
  }) => {
...
test.describe("FAQ Visual Regression", () => {
  test("FAQ section collapsed state on mobile", async ({ page }) => {
    ...
    await page
      .locator("text=Veelgestelde vragen")
      .first()
      .scrollIntoViewIfNeeded();

    // Take screenshot of FAQ section
    await expect(page.locator(".faq-card").first()).toHaveScreenshot(
      "faq-collapsed-mobile.png",
    );
  });
