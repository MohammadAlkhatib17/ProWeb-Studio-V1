 ...
 */
export function scanImagesInDirectory(dirPath: string, baseUrl: string = SITE_URL): ImageSitemapEntry[] {
  const imageExtensions = ['.jpg', '.jpeg', '.png', '.webp', '.avif', '.svg', '.gif'];
  ...
  function scanDirectory(currentPath: string, urlPath: string = ''): void {
    try {
      ...
      for (const item of items) {
        ...
        if (item.isDirectory() && !item.name.startsWith('.')) {
        ...
        } else if (item.isFile() && imageExtensions.includes(extname(item.name).toLowerCase())) {
          ...
          const imageEntry: ImageSitemapEntry = {
            ...
            lastModified: getLastModifiedDate(itemPath.replace(process.cwd() + '/', '')),
          };
 ...
 */
export function scanVideosInDirectory(dirPath: string, baseUrl: string = SITE_URL): VideoSitemapEntry[] {
  ...
  function scanDirectory(currentPath: string, urlPath: string = ''): void {
    try {
      ...
      for (const item of items) {
        ...
        if (item.isDirectory() && !item.name.startsWith('.')) {
          scanDirectory(itemPath, itemUrlPath);
        } else if (item.isFile() && videoExtensions.includes(extname(item.name).toLowerCase())) {
          const pageUrl = findAssociatedPage(itemUrlPath);
          ...
          const videoEntry: VideoSitemapEntry = {
            ...
            videos: [{
              ...
              thumbnailLoc: findVideoThumbnail(itemPath) || `${baseUrl}/assets/video-thumbnail-default.jpg`,
              title: generateVideoTitle(item.name),
              description: generateVideoDescription(item.name),
              publicationDate: getLastModifiedDate(itemPath.replace(process.cwd() + '/', '')),
              familyFriendly: true,
              live: false,
            }],
            lastModified: getLastModifiedDate(itemPath.replace(process.cwd() + '/', '')),
 ...
 */
export function chunkSitemapEntries<T>(entries: T[], maxPerSitemap: number = 50000): T[][] {
  const chunks: T[][] = [];
...
export function generateImageSitemapXML(entries: ImageSitemapEntry[]): string {
  ...
  const urls = entries.map(entry => {
    const lastmod = entry.lastModified ? entry.lastModified.toISOString().split('T')[0] : new Date().toISOString().split('T')[0];
    let urlXml = `  <url>
    <loc>${entry.url}</loc>
    <lastmod>${lastmod}</lastmod>`;
...
export function generateSitemapIndexXML(sitemaps: Array<{ loc: string; lastmod?: Date }>): string {
  ...
  const sitemapEntries = sitemaps.map(sitemap => {
    const lastmod = sitemap.lastmod ? sitemap.lastmod.toISOString().split('T')[0] : new Date().toISOString().split('T')[0];
    return `  <sitemap>
    <loc>${sitemap.loc}</loc>
    <lastmod>${lastmod}</lastmod>
  </sitemap>`;
