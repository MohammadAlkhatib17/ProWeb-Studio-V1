...
export class NotFoundMonitor {
   ...
   */
  async generateRedirectSuggestions(
    notFoundUrl: string,
  ...
  getAnalytics(days: number = 30): NotFoundAnalytics {
    ...
    const recentData = Array.from(this.notFoundData.values()).filter(
  ...
  getRedirectSuggestions(
    limit: number = 10,
  ): Array<{ url: string; suggestions: RedirectSuggestion[] }> {
  ...
  async detectBrokenLinks(
    siteUrl: string,
  ): Promise<Array<{ page: string; brokenLinks: string[] }>> {
  ...
  private shouldIgnoreUrl(url: string): boolean {
    const ignorePatterns =
      MONITORING_CONFIG.NOT_FOUND_MONITORING.IGNORE_PATTERNS;
  ...
  private checkForRedirectOpportunity(url: string): void {
    ...
    this.generateRedirectSuggestions(url)
      .then((suggestions) => {
  ...
  private checkAlertThreshold(url: string): void {
    ...
    const threshold =
      MONITORING_CONFIG.NOT_FOUND_MONITORING.FREQUENCY_THRESHOLD;
  ...
  private analyzeReferrers(url: string): string[] {
    ...
    if (entry.referer.includes("google.com")) {
      ...
      analysis.push(
        "External referrer - consider creating content or redirect",
      );
    }
      ...
      if (matchingUrls.length > 0) {
        suggestions.push({
          ...
          evidence: {
            ...
            referrerAnalysis: [
              `Category "${category}" matches existing content`,
            ],
    ...
    if (entry.referer.includes(process.env.NEXT_PUBLIC_SITE_URL || "")) {
      suggestions.push({
        ...
        evidence: {
          ...
          referrerAnalysis: [
            "Internal link - redirect to homepage as fallback",
          ],
        },
      });
    }
  ...
  private getTopMissingPages(
    data: NotFoundMonitoring[],
  ): Array<{ url: string; hits: number; lastSeen: number }> {
    return data
      .sort((a, b) => b.frequency - a.frequency)
      .slice(0, 10)
      .map((item) => ({
  ...
  private getTopReferrers(
    data: NotFoundMonitoring[],
  ): Array<{ referrer: string; hits: number }> {
  ...
  private getSuggestedAction(
    pattern: string,
    urls: string[],
  ): NotFoundPattern["suggestedAction"] {
    ...
    if (urls.some((url) => this.siteUrls.has(url.replace(/\/$/, ""))))
      return "redirect";
    return "fix-links";
  }
}
