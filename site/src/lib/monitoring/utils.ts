...
export class PerformanceMonitor {
  ...
  recordMetric(metric: CoreWebVitalsMetrics): void {
    ...
    const cutoff =
      Date.now() -
      MONITORING_CONFIG.CORE_WEB_VITALS.RETENTION_DAYS * 24 * 60 * 60 * 1000;
  ...
  checkThresholds(metric: CoreWebVitalsMetrics): PerformanceAlert[] {
    ...
    if (metric.lcp > thresholds.LCP.NEEDS_IMPROVEMENT) {
      alerts.push({
        ...
        severity:
          metric.lcp > thresholds.LCP.NEEDS_IMPROVEMENT * 1.5
            ? "critical"
            : "high",
        timestamp: Date.now(),
        url: metric.url,
    ...
    if (metric.fid > thresholds.FID.NEEDS_IMPROVEMENT) {
      alerts.push({
        ...
        severity:
          metric.fid > thresholds.FID.NEEDS_IMPROVEMENT * 2
            ? "critical"
            : "high",
        timestamp: Date.now(),
        url: metric.url,
    ...
    if (metric.cls > thresholds.CLS.NEEDS_IMPROVEMENT) {
      alerts.push({
        ...
        severity:
          metric.cls > thresholds.CLS.NEEDS_IMPROVEMENT * 2
            ? "critical"
            : "high",
        timestamp: Date.now(),
        url: metric.url,
...
export class SEOHealthMonitor {
  static calculateSEOScore(check: SEOHealthCheck): number {
    ...
    // Meta tags score
    const metaScore =
      Object.values(check.checks.metaTags).filter(Boolean).length /
    ...
    const indexabilityScore =
      indexabilityFactors.filter(Boolean).length / indexabilityFactors.length;
    ...
    const ogScore =
      Object.values(check.checks.socialSharing.openGraph).filter(Boolean)
        .length / 5;
    const twitterScore =
      Object.values(check.checks.socialSharing.twitter).filter(Boolean).length /
      4;
  ...
  static detectCriticalIssues(check: SEOHealthCheck): SEOIssue[] {
    ...
    return check.issues.filter((issue) =>
      critical.some((criticalPattern) =>
        issue.message.includes(criticalPattern),
      ),
    );
  }
}
  ...
  static findSimilarUrls(
    ...
    return candidateUrls
      ...
      .filter(
        (item) =>
          item.similarity >=
          MONITORING_CONFIG.NOT_FOUND_MONITORING.SIMILARITY_THRESHOLD,
      )
...
export class AlertManager {
  ...
  static createAlert(
    ...
    if (
      lastAlert &&
      now - lastAlert < MONITORING_CONFIG.ALERTS.THROTTLING.SAME_ALERT_INTERVAL
    ) {
