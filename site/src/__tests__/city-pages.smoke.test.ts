...
vi.mock('@/components/RelatedServices', () => ({
  default: () => React.createElement('div', { 'data-testid': 'related-services' }),
}));

vi.mock('@/components/ContentSuggestions', () => ({
  default: () => React.createElement('div', { 'data-testid': 'content-suggestions' }),
}));
  ...
  testCities.forEach((citySlug) => {
    describe(`City: ${citySlug}`, () => {
      ...
      it('should render with homepage hero section classes', () => {
        ...
        expect(heroSection).toHaveClass('relative', 'min-h-[92vh]', 'grid', 'place-items-center', 'overflow-hidden');

        // Check for homepage-style layout classes
        const mainElement = container.querySelector('main');
        expect(mainElement).toHaveClass('relative', 'content-safe-top', 'overflow-hidden');
      });

      it('should have homepage-style CTA buttons', () => {
        render(React.createElement(LocationPage, { params: { location: citySlug } }));

        // Check for primary CTA (contact)
        const contactButton = screen.getByRole('link', { name: /start uw project/i });
        expect(contactButton).toBeTruthy();
        expect(contactButton.getAttribute('href')).toBe('/contact');

        // Check for secondary CTA (portfolio)
        const portfolioButton = screen.getByRole('link', { name: /bekijk portfolio/i });
