    ...
    <Suspense fallback={fallbackContent || defaultFallback}>
      <Canvas
        ...
        gl={{
          ...
          powerPreference:
            canvasSettings.powerPreference as WebGLPowerPreference,
        }}
      >
      ...
      try {
        ...
        const priorityLoaded = await Promise.allSettled(
          priorityAssets.map(async (path) => {
            // Load asset using appropriate loader based on file extension
            const extension = path.split(".").pop()?.toLowerCase();
            if (extension === "gltf" || extension === "glb") {
              // Use useGLTF from @react-three/drei for GLTF loading
              return fetch(path)
                .then((res) => res.arrayBuffer())
                .then((buffer) => ({ path, buffer, loaded: true }));
            } else if (
              extension === "jpg" ||
              extension === "jpeg" ||
              extension === "png" ||
              ...
              // Preload texture by creating an Image element
              return new Promise((resolve, reject) => {
                const img = new Image();
                img.onload = () =>
                  resolve({ path, element: img, loaded: true });
                img.onerror = reject;
                img.src = path;
              });
            }
        ...
        setLoadedAssets((prev) => ({
          ...prev,
          ...Object.fromEntries(
            priorityLoaded.map((result, index) => [
        ...
        setTimeout(async () => {
          ...
          setLoadedAssets((prev) => ({
            ...prev,
            ...Object.fromEntries(
              deferredLoaded.map((result, index) => [
