...
function createSafeEmailContent(data: ContactData) {
  const sanitizedData = {
    ...
    phone: data.phone ? sanitizeText(data.phone) : 'Niet opgegeven',
    projectTypes: data.projectTypes.map((type: string) => sanitizeText(type)),
...
export async function POST(req: NextRequest) {
  try {
    ...
    const info = await transporter.sendMail({
      ...
      subject: `Nieuwe aanvraag van ${sanitizeText(name)}`,
      replyTo: email,
      html,
      headers: {
        'X-Priority': '3',
