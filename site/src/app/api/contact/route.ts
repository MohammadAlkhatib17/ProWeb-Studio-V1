...
/**
 * Create unified error response with security headers
 * Never exposes internal details to prevent information leakage
 */
...
function createSuccessResponse(
  ...
  const response = NextResponse.json({ ok: true, message }, { headers });
...
export async function POST(req: NextRequest) {
  try {
    ...
    const clientIP =
      ...
      "unknown";
    const rateLimitIdentifier = createRateLimitIdentifier(req);

    // 1. Validate request security (Origin, Host, Fetch metadata)
    const securityValidation = validateRequestSecurity(req);
    if (!securityValidation.valid) {
      console.warn(
        `Security validation failed from ${clientIP}: ${securityValidation.error} (${securityValidation.code})`,
      );
      return createErrorResponse(
        "Verzoek niet toegestaan vanuit deze bron",
        403,
      );
    }
    ...
    if (!rateLimitResult.allowed) {
      console.warn(`Rate limit exceeded from ${clientIP}`);
      return createErrorResponse(
    ...
    if (!parsed.success) {
      ...
      return createErrorResponse(
        "Formulier gegevens zijn ongeldig. Controleer alle velden en probeer opnieuw.",
        400,
        rateLimitHeaders,
      );
    }
    ...
    // Always return generic error message to client (no internal details)
    return createErrorResponse(
