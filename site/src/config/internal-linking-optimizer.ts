...
export function analyzeInternalLinking(): LinkEquityAnalysis {
  ...
  const highPriorityLinks = allLinks.filter(
    (l) => l.priority === "high",
  ).length;
  ...
  if (highPriorityLinks < 5) {
    recommendations.push(
      "Voeg meer high-priority links toe aan belangrijke service pagina's",
    );
  }

  if (totalLinks > 100) {
    recommendations.push(
      "Overweeg het reduceren van het totaal aantal links om link equity te concentreren",
    );
  }

  const deepLinks = Object.entries(linkDepth).filter(
    ([, depth]) => depth > 3,
  ).length;
  if (deepLinks > totalLinks * 0.3) {
    recommendations.push(
      "Te veel diepe links - overweeg het verplaatsen van belangrijke content naar hogere niveaus",
    );
  }
...
export function generatePageLinkStrategy(pathname: string): PageLinkStrategy {
  ...
  baseStrategy.requiredLinks = ["/", "/diensten", "/portfolio", "/contact"];
    ...
    case pathname.startsWith("/locaties"):
      ...
      if (currentLocationData) {
        ...
        baseStrategy.suggestedLinks = [
          ...
          ...currentLocationData.nearbyLocations
            .slice(0, 3)
            .map((l) => `/locaties/${l}`),
          "/contact",
          "/portfolio",
...
export function validateInternalLinks(): Array<{
  issue: string;
  ...
  locations.forEach((location) =>
    allLinkedPages.add(`/locaties/${location.slug}`),
  );
  ...
  services.forEach((service) => {
    if (service.relatedServices.length === 0) {
      issues.push({
        ...
        severity: "medium",
        recommendation:
          "Voeg gerelateerde services toe om cross-linking te verbeteren",
      });
    }
  });
  ...
  locations.forEach((location) => {
    if (location.nearbyLocations.length === 0) {
      issues.push({
        ...
        severity: "low",
        recommendation:
          "Voeg nabijgelegen locaties toe voor betere geografische linking",
      });
    }
  });
