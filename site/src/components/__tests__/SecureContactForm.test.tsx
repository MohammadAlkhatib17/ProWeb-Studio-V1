...
describe("SecureContactForm", () => {
  ...
  it("should render the contact form with honeypot field", () => {
    ...
    expect(
      screen.getByLabelText(/Vertel ons over jouw project/),
    ).toBeInTheDocument();
  ...
  it("should validate required fields", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Naam moet minimaal 2 karakters zijn/),
      ).toBeInTheDocument();
      expect(screen.getByText(/Ongeldig e-mailadres/)).toBeInTheDocument();
      expect(
        screen.getByText(/Selecteer minimaal één projecttype/),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/Bericht moet minimaal 10 karakters zijn/),
      ).toBeInTheDocument();
    });
