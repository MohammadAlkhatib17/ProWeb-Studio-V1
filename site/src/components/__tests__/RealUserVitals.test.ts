...
const testRealUserVitals = () => {
  ...
  const testRating = (metric: string, value: number, expected: string) => {
    ...
    if (process.env.NODE_ENV !== "production") {
      console.log(
        `${passed ? "✅" : "❌"} ${metric} ${value} → ${rating} (expected: ${expected})`,
      );
    }
    return passed;
  };
  ...
  const testBucketing = (
    ...
    if (process.env.NODE_ENV !== "production") {
      console.log(
        `${passed ? "✅" : "❌"} ${metric} ${value} → ${bucket} (expected: ${expectedBucket})`,
      );
    }
    return passed;
  };
  ...
  const testSampling = () => {
    ...
    if (process.env.NODE_ENV !== "production") {
      console.log(
        `✅ Sampling logic: 0.1 < 0.2 = ${shouldSample1}, 0.5 < 0.2 = ${shouldSample2}`,
      );
    }
  };
