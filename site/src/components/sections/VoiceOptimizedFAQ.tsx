  ...
  const filteredFAQs = serviceName
    ? VOICE_SEARCH_FAQS.filter((faq) =>
        faq.keywords.some((keyword) =>
          keyword
            .toLowerCase()
            .includes(serviceName.toLowerCase().replace("-", " ")),
        ),
      )
    : VOICE_SEARCH_FAQS;
        ...
        <header className="text-center mb-16">
          ...
          >
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-magenta-400">
              Website Laten Maken
              {cityName ? ` in ${cityName}` : " in Nederland"}
            </span>
          </SemanticHeading>
          <p className="text-xl text-slate-300">
            Alle antwoorden op uw vragen over{" "}
            <strong>professionele webdesign</strong> en
        ...
        {!showAll && filteredFAQs.length > 6 && (
          <div className="text-center mt-12">
            <p className="text-slate-300 mb-4">
              Heeft u nog meer vragen over <strong>website laten maken</strong>{" "}
              of
...
export function VoiceSearchOptimizedContent({
  ...
  const localizedKeywords = cityName
    ? keywords.map((k) =>
        k.includes("Nederland")
          ? k.replace("Nederland", cityName)
          : `${k} ${cityName}`,
      )
    : keywords;
