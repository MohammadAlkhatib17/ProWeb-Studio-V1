...
} from "../seo/DutchKeywordOptimization";
...
export function LocationSpecificHero({
  city,
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <KeywordOptimizedHeading
              ...
              secondaryKeywords={[
                `webdesign ${city.name}`,
                `website ontwikkeling ${city.name}`,
              ]}
              cityName={city.name}
            ...
            >
              ...
              <span className="block text-2xl sm:text-3xl lg:text-4xl mt-4 text-slate-200">
                {city.characteristics[0] &&
                  `${city.characteristics[0].charAt(0).toUpperCase() + city.characteristics[0].slice(1)} - `}
            ...
            >
              <p className="text-lg sm:text-xl lg:text-2xl text-slate-300 mb-8 max-w-4xl mx-auto leading-relaxed">
                <strong>Website laten maken in {city.name}</strong>? ProWeb
                Studio is uw lokale <strong>webdesign specialist</strong> voor{" "}
                <strong>professionele websites</strong> en{" "}
                <strong>webshops</strong> in {city.region}.
                {city.businessFocus && city.businessFocus.length > 0 && (
                  <span>
                    {" "}
                    Gespecialiseerd in{" "}
                    {city.businessFocus.slice(0, 2).join(" en ")} sectoren.
                  </span>
                )}
              </p>
            </DutchBusinessTerminology>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
              <div className="bg-cosmic-800/40 p-6 rounded-xl border border-cosmic-700/60">
                ...
                <p className="text-slate-300 text-sm">
                  ...
                  {city.localTerms && city.localTerms[0]
                    ? city.localTerms[0]
                    : `${city.name} ondernemers`}{" "}
                  nodig hebben voor online succes.
                </p>
              </div>
              ...
              <div className="bg-cosmic-800/40 p-6 rounded-xl border border-cosmic-700/60">
                ...
                <p className="text-slate-300 text-sm">
                  <strong>Lokale SEO</strong> geoptimaliseerd voor {city.name},{" "}
                  {city.region} en omliggende gemeenten voor maximale regionale
                  vindbaarheid.
                </p>
              </div>
              ...
              <div className="bg-cosmic-800/40 p-6 rounded-xl border border-cosmic-700/60">
                ...
                <p className="text-slate-300 text-sm">
                  Ervaring met{" "}
                  {city.keyIndustries?.slice(0, 2).join(" en ") ||
                    "lokale bedrijven"}{" "}
...
export function LocationSpecificServices({
  city,
  className = "",
}: LocationSpecificContentProps) {
  const cityServices = [
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <header className="text-center mb-16">
          <SemanticHeading
            ...
            keywords={[
              `webdesign diensten ${city.name}`,
              `website services ${city.name}`,
            ]}
          >
          ...
          <p className="text-xl text-slate-300 max-w-3xl mx-auto">
            Professionele webdesign services voor {city.name} ondernemers en
            bedrijven. Lokale expertise, Nederlandse kwaliteit.
          </p>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {cityServices.map((service, index) => (
            <article
              key={index}
              className="bg-cosmic-800/40 p-8 rounded-2xl border border-cosmic-700/60 hover:border-cosmic-600/80 transition-all duration-300 hover:shadow-2xl hover:shadow-cyan-500/10"
            >
        ...
        <div className="text-center mt-16">
          <div className="bg-cosmic-800/40 p-8 rounded-2xl border border-cosmic-700/60 max-w-4xl mx-auto">
            ...
            <p className="text-slate-300 mb-6">
              Klaar om uw online aanwezigheid in {city.name} te versterken? Neem
              contact op voor een vrijblijvend gesprek over uw website project.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Button href={`/contact?city=${city.slug}`} variant="primary">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <header className="text-center mb-16">
          ...
          <p className="text-xl text-slate-300">
            Ontdek ervaringen van {city.name} bedrijven die hun website door ons
            lieten maken
          </p>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {cityTestimonials.map((testimonial, index) => (
            <article
              key={index}
              className="bg-cosmic-800/40 p-8 rounded-2xl border border-cosmic-700/60"
            >
            ...
            <div className="grid grid-cols-3 gap-6 text-center">
              <div>
                <div className="text-2xl font-bold text-cyan-300">
                  {Math.floor(city.population! / 10000)}+
                </div>
                <div className="text-sm text-slate-400">
                  Websites in {city.region}
                </div>
              </div>
              <div>
                <div className="text-2xl font-bold text-cyan-300">98%</div>
                <div className="text-sm text-slate-400">
                  Tevreden {city.name} klanten
                </div>
              </div>
              <div>
