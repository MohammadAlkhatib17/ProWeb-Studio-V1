    ...
    if (currentPage === "service" && currentSlug) {
      ...
      return {
        ...
        locations: locations.filter(
          (l) => currentService?.targetLocation?.includes(l.slug) || false,
        ),
        currentService,
      };
    }
    ...
    if (currentPage === "location" && currentSlug) {
      ...
      return {
        services: services.filter(
          (s) =>
            currentLocation?.relatedServices.some((rs) =>
              s.href.includes(rs),
            ) || false,
        ),
        locations: locations.filter(
          (l) => currentLocation?.nearbyLocations.includes(l.slug) || false,
        ),
        currentLocation,
      };
    }
          ...
          {currentPage === "location" && currentLocation && (
            <>
              ...
              <li className="flex items-center">
                <span className="mx-2">→</span>
                <span className="text-cyan-300 font-medium">
                  Webdesign {currentLocation.name}
                </span>
              </li>
            </>
          )}
        ...
        >
          ...
          >
            {currentPage === "location"
              ? `Onze Diensten in ${currentLocation?.name}`
              : "Onze Webdesign Diensten"}
          </SemanticHeading>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {relatedServices.map((service, index) => (
              <article
                key={index}
                className="bg-cosmic-800/40 p-6 rounded-xl border border-cosmic-700/60 hover:border-cosmic-600/80 transition-all duration-300"
              >
                <SemanticHeading
                  level={3}
                ...
                <p className="text-slate-300 text-sm mb-4 leading-relaxed">
                  {service.description}
                  {currentLocation &&
                    ` Speciaal voor bedrijven in ${currentLocation.name} en omgeving.`}
                </p>
                <Link
                  href={service.href}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {relevantLocations.map((location, index) => (
            ...
            >
              ...
              <div className="text-xs text-slate-400 mb-4">
                <strong>Regio:</strong> {location.region} |{" "}
                <strong>Inwoners:</strong>{" "}
                {location.population?.toLocaleString("nl-NL")}
              </div>
        ...
        <div className="text-center mt-12">
          <p className="text-slate-300 mb-4">
            Wij bedienen bedrijven door heel Nederland met lokale expertise en
            persoonlijke service.
          </p>
          <Button href="/locaties" variant="primary">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {relevantServices.map((service, index) => (
            <article
              key={index}
              className="bg-cosmic-800/40 p-8 rounded-xl border border-cosmic-700/60 hover:border-cosmic-600/80 transition-all duration-300"
            >
              <SemanticHeading
                level={3}
              ...
              <p className="text-slate-300 mb-6 leading-relaxed">
                {service.description} Speciaal voor bedrijven in {cityName}{" "}
                bieden wij lokale expertise gecombineerd met Nederlandse
                webdesign kwaliteit.
              </p>
                ...
                <div className="bg-cosmic-900/40 p-4 rounded-lg mb-6">
                  ...
                  <div className="text-xs text-slate-400">
                    {currentLocation.keyIndustries.map((industry: string) => (
                      <span
                        key={industry}
                        className="inline-block bg-cosmic-700/40 px-2 py-1 rounded mr-2 mb-1"
                      >
                        {industry}
                      </span>
                    ))}
