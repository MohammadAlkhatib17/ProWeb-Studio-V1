...
function detectGPUCapabilities(): Promise<{
  gpuTier: "low" | "medium" | "high";
  maxTextureSize: number;
}> {
...
function getPerformanceTier(
  ...
  if (isMobile) {
    if (gpuTier === "low" || hardwareConcurrency < 4 || deviceMemory < 4)
      return "low";
  ...
  if (gpuTier === "low" || hardwareConcurrency < 4 || deviceMemory < 8)
    return "low";
  if (gpuTier === "high" && hardwareConcurrency >= 8 && deviceMemory >= 16)
    return "high";
  return "medium";
}
...
export default function Dynamic3DWrapper({
  ...
  useEffect(() => {
    ...
    if (
      performanceState.metrics.fps > 40 &&
  ...
  if (!hasWebGL || !shouldRender3D) {
    return (
      <div
        className={`${className} flex items-center justify-center bg-gradient-to-br from-gray-900 to-black`}
      >
        <div className="text-center p-6 sm:p-7 md:p-8">
          <p className="text-slate-400">
            {!hasWebGL
              ? "WebGL is not supported on this device"
              : "3D content is disabled due to performance or accessibility settings"}
          </p>
          <p className="text-sm text-gray-500 mt-2">
            3D content has been disabled
          </p>
        </div>
      </div>
    );
  ...
  return (
    <PerformanceContext.Provider value={performanceState}>
      <ThreeErrorBoundary variant={variant}>
        <Suspense
          fallback={<LoadingSkeleton variant={variant} className={className} />}
        >
