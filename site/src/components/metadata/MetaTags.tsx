...
export function PaginationMetaTags({ current, total, baseUrl }: PaginationMetaTagsProps) {
...
export function HrefLangTags({ 
  ...
  return (
    <>
      ...
      <link rel="alternate" hrefLang="nl-NL" href={`${baseUrl}${currentPath}`} />
      
      {/* Location-specific hreflang for better geo-targeting */}
      {locations.map(location => {
        const locationData = locationMetadata[location as keyof typeof locationMetadata];
        if (locationData) {
          return (
            <link
              key={location}
    ...
    <>
      ...
      {expirationTime && (
        <meta 
          ...
          content={expirationTime instanceof Date ? expirationTime.toISOString() : expirationTime} 
        />
      )}
      <meta name="date" content={modifiedISO} />
      <meta name="last-modified" content={modifiedISO} />
      <meta httpEquiv="last-modified" content={new Date(modifiedISO).toUTCString()} />
    </>
  );
}
...
export function BusinessMetaTags({
  ...
  return (
    <>
      ...
      <meta name="business:contact_data:website" content="https://prowebstudio.nl" />
