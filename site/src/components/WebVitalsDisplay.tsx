...
export function WebVitalsDisplay({
  ...
  const [metrics, setMetrics] = useState<
    Array<{ name: string; value: number; rating: string; id: string }>
  >([]);
  ...
  return (
    <>
      ...
      {isVisible && (
        <div
          className={`fixed ${positionClasses[position]} z-40 bg-gray-900/95 backdrop-blur-sm text-white p-4 rounded-lg text-xs font-mono w-80 max-h-96 overflow-y-auto mt-12`}
        >
              ...
              return (
                ...
                >
                  ...
                  <div className="text-right">
                    <div
                      className={`${metric ? getMetricColor(metric.rating) : "text-gray-500"}`}
                    >
          ...
          {metrics.some((m) => m.name.includes("_TIME")) && (
            <div className="space-y-2">
              ...
              {metrics
                .filter((m) => m.name.includes("_TIME"))
                .map((metric) => (
                  <div
                    key={metric.name}
                    className="flex justify-between items-center"
