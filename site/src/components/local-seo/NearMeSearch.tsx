...
export default function NearMeSearch({ currentCity, className = '' }: NearMeSearchProps) {
      ...
      (error) => {
        ...
        switch (error.code) {
          case error.PERMISSION_DENIED:
            errorMessage = 'Locatietoegang geweigerd. Schakel locatiediensten in om nabijgelegen steden te vinden.';
            break;
  ...
  const suggestedCities = locationState.nearestCities.length > 0 
    ? locationState.nearestCities
    : getNearestCities(currentCity.coordinates, 5).filter(city => city.slug !== currentCity.slug);
  ...
  return (
    <div className={`bg-white dark:bg-gray-900 rounded-lg p-6 shadow-lg ${className}`}>
      ...
      <p className="text-gray-600 dark:text-gray-300 mb-6 text-sm">
        Vind de dichtstbijzijnde stad waar wij onze webdesign diensten aanbieden.
      </p>
      ...
      {locationState.nearbyCity && (
        <div className="mb-6 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
          ...
          <p className="text-sm text-green-700 dark:text-green-300 mb-3">
            Wij bieden onze diensten aan in <strong>{locationState.nearbyCity.name}</strong>, {locationState.nearbyCity.province}
          </p>
          <a
      ...
      <div>
        <h4 className="font-medium text-gray-900 dark:text-white mb-3 text-sm">
          {locationState.nearestCities.length > 0 ? 'Andere Nabijgelegen Steden' : 'Steden in de Buurt'}
        </h4>
            ...
            >
              <div>
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400">
                  {city.province} • {city.population.toLocaleString('nl-NL')} inwoners
                </div>
              </div>
              <svg className="w-4 h-4 text-gray-400 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors" fill="currentColor" viewBox="0 0 20 20">
