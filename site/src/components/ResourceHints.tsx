...
export function ResourceHints({
  ...
  const routePrefetches = enableRoutePrefetch
    ? ROUTE_PREFETCH_CONFIG[currentPath] || []
    : [];
  ...
  return (
    <>
      ...
      {enableDNSPrefetch &&
        DNS_PREFETCH_DOMAINS.map((domain, index) => (
          <link
      ...
      {CRITICAL_PRECONNECTS.filter(
        (config) => config.type === "dns-prefetch",
      ).map((config, index) => (
        <link
